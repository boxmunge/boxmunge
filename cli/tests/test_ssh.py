...
class TestBuildSshCmd:
    def test_basic_command(self) -> None:
        ...
        assert cmd == ["ssh", "-p", "922", "deploy@box.example.com", "stage", "myapp"]

    def test_extra_args(self) -> None:
        config = {"server": "box.example.com", "port": 922, "user": "deploy", "project": "myapp"}
        cmd = build_ssh_cmd(config, "prod-deploy", "myapp", "--dry-run")
        assert cmd == ["ssh", "-p", "922", "deploy@box.example.com", "prod-deploy", "myapp", "--dry-run"]
    ...
    def test_custom_port_and_user(self) -> None:
        ...
        assert cmd == ["ssh", "-p", "2222", "admin@10.0.0.1", "status", "myapp"]

    def test_no_project_arg(self) -> None:
        config = {"server": "box.example.com", "port": 922, "user": "deploy", "project": "myapp"}
        cmd = build_ssh_cmd(config, "handshake")
        assert cmd == ["ssh", "-p", "922", "deploy@box.example.com", "handshake"]
...
class TestBuildScpCmd:
    def test_basic_upload(self) -> None:
        ...
        assert cmd == ["scp", "-P", "922", "/tmp/myapp-2024.tar.gz", "deploy@box.example.com:"]
...
class TestRunSsh:
    ...
    def test_passes_args_as_list(self, mock_run: MagicMock) -> None:
        ...
        assert call_args == ["ssh", "-p", "922", "deploy@box.example.com", "status", "myapp"]
        assert code == 0
...
class TestRunScp:
    ...
    def test_uploads_file(self, mock_run: MagicMock) -> None:
        ...
        call_args = mock_run.call_args[0][0]
        assert call_args == ["scp", "-P", "922", "/tmp/bundle.tar.gz", "deploy@box.example.com:"]
        assert code == 0
